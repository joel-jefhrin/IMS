...
export default function MappingsPage() {
  ...
  const questionsWithDepts = useMemo(
    () =>
      questions.map((q: any) => {
        ...
        const usedInCampaigns = campaigns.filter(
          (c) => c.questionSetIds && c.questionSetIds.includes(q.id)
        ).length;
        return {
          ...q,
    ...
    () =>
      candidates.map((can) => {
        ...
        const dept = departments.find(
          (d) => d.id === can.preferredDepartmentId
        );
        return {
          ...can,
          ...
          departmentName: dept?.name || "Unknown",
          assignedQuestions: can.assignedQuestions
            ? can.assignedQuestions.length
            : 0,
        };
      }),
  ...
  const filteredQuestions = questionsWithDepts.filter(
    (q) =>
      q.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
      q.departmentName.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const filteredCampaigns = campaignsWithDetails.filter(
    (c) =>
  ...
  const deptQuestions = selectedDeptId
    ? questionsWithDepts.filter((q) => q.departmentId === selectedDeptId)
    : [];
  const deptCampaigns = selectedDeptId
    ? campaignsWithDetails.filter((c) => c.departmentId === selectedDeptId)
    : [];
  const selectedDept = departmentsWithCounts.find(
    (d) => d.id === selectedDeptId
  );

  // Get data for selected campaign
  const campaignQuestions = selectedCampaignId
    ? questionsWithDepts.filter(
        (q) =>
          campaignsWithDetails.find((c) => c.id === selectedCampaignId)
            ?.departmentId === q.departmentId
      )
    : [];
  const campaignCandidates = selectedCampaignId
    ? candidatesWithDetails.filter((c) => c.campaignId === selectedCampaignId)
    : [];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            View connections between Departments, Campaigns, Questions &
            Candidates
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-2 flex-wrap">
        <button
          ...
          className={`btn-sm ${
            selectedView === "overview" ? "btn" : "btn-outline"
          }`}
        >
          Overview
        </button>
        <button
          ...
          className={`btn-sm ${
            selectedView === "department" ? "btn" : "btn-outline"
          }`}
        >
          By Department
        </button>
        <button
          ...
          className={`btn-sm ${
            selectedView === "campaign" ? "btn" : "btn-outline"
          }`}
        >
          By Campaign
        </button>
        <button
          ...
          className={`btn-sm ${
            selectedView === "question" ? "btn" : "btn-outline"
          }`}
        >
          By Question
        </button>
            ...
            <div className="mt-8 p-4 bg-blue-50 rounded-lg border border-blue-200">
              ...
              <ul className="text-sm text-blue-800 space-y-2">
                <li className="flex items-start gap-2">
                  ...
                  <span>
                    <strong>Departments</strong> are created (e.g., Engineering,
                    Design)
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  ...
                  <span>
                    <strong>Questions</strong> are tagged with a specific
                    department
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  ...
                  <span>
                    <strong>Campaigns</strong> select questions from their
                    department's question pool
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-blue-600 font-bold">4.</span>
                  <span>
                    <strong>Candidates</strong> are assigned to a campaign and
                    receive its questions
                  </span>
                </li>
              </ul>
            </div>
                    ...
                    <tr key={dept.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-4">
                        <span className="badge badge-success">
                          {dept.campaignCount} campaigns
                        </span>
                      </td>
                      <td className="px-4 py-4">
                        <span className="badge badge-info">
                          {dept.candidateCount} candidates
                        </span>
                      </td>
                      <td className="px-4 py-4">
                        <button
              ...
              <div className="card p-6 bg-gradient-to-br from-primary-50 to-primary-100 border-2 border-primary-200">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-gray-600 mt-1">
                      Department Details & Relationships
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-3xl font-bold text-primary-600">
                      {selectedDept.questionCount +
                        selectedDept.campaignCount +
                        selectedDept.candidateCount}
                    </p>
                    ...
                    >
                      ...
                      <div className="flex gap-2 mt-2">
                        ...
                        <span className="text-xs text-gray-600">
                          Used in {q.usedInCampaigns} campaigns
                        </span>
                      </div>
                    </div>
                  ))}
                  ...
                  {deptCampaigns.map((c) => (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm text-gray-600 mt-1">
                          {c.questionCount} questions • {c.candidateCount}{" "}
                          candidates
                        </p>
                      </div>
                      <span
            ...
            <>
              ...
              <div className="grid md:grid-cols-3 gap-4">
                <div className="card p-4 text-center">
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    {selectedCampaign.questionCount}
                  </p>
              ...
              <div className="card p-6">
                ...
                <p className="text-sm text-gray-600 mb-4">
                  Questions from{" "}
                  <strong>{selectedCampaign.departmentName}</strong> department
                  that can be used in this campaign:
                </p>
              ...
              <div className="card p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">
                  Candidates ({campaignCandidates.length})
                </h3>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-sm text-gray-600">
                          {can.assignedQuestions} questions
                        </span>
                        <span
                          className={`badge ${
                            can.status === "completed"
                              ? "badge-success"
                              : can.status === "in_progress"
                              ? "badge-warning"
                              : "badge-secondary"
                          }`}
                        >
