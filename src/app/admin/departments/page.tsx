...
export default function DepartmentsPage() {
  ...
  const departmentsWithStats: DepartmentWithStats[] = departments.map(
    (dept) => {
  ...
  const handleDelete = (id: string) => {
    ...
    if (dept && (dept.campaigns || 0) > 0) {
      toast.error(
        `Cannot delete ${dept.name}: ${dept.campaigns} active campaigns`
      );
      return;
    }
  ...
  // Calculate totals
  const totalQuestions = departmentsWithStats.reduce(
    (sum, d) => sum + (d.questionSets || 0),
    0
  );
  const totalCampaigns = departmentsWithStats.reduce(
    (sum, d) => sum + (d.campaigns || 0),
    0
  );
  const totalCandidates = departmentsWithStats.reduce(
    (sum, d) => sum + (d.candidates || 0),
    0
  );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Manage departments and evaluation criteria
          </p>
        </div>
            ...
            <div className="grid grid-cols-3 gap-2 mb-4 text-center">
              <div className="bg-gray-50 p-2 rounded">
                ...
                <p className="text-lg font-bold text-gray-900">
                  {department.questionSets || 0}
                </p>
              </div>
              <div className="bg-gray-50 p-2 rounded">
                <p className="text-xs text-gray-600">Campaigns</p>
                <p className="text-lg font-bold text-gray-900">
                  {department.campaigns || 0}
                </p>
              </div>
              <div className="bg-gray-50 p-2 rounded">
              ...
              <div className="space-y-1">
                <div className="flex justify-between text-xs">
                  ...
                  <span className="font-medium text-gray-900">
                    {department.evaluationCriteria?.technicalWeight || 0}%
                  </span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-gray-600">Behavioral</span>
                  <span className="font-medium text-gray-900">
                    {department.evaluationCriteria?.behavioralWeight || 0}%
                  </span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-gray-600">Logical</span>
                  <span className="font-medium text-gray-900">
                    {department.evaluationCriteria?.logicalWeight || 0}%
                  </span>
                </div>
              </div>
              <div className="mt-2 pt-2 border-t border-gray-200">
                <div className="flex justify-between text-xs">
                  ...
                  <span className="font-semibold text-gray-900">
                    {department.evaluationCriteria?.passingScore || 70}%
                  </span>
                </div>
              </div>
            </div>
