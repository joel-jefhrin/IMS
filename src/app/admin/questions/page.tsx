...
const mockQuestions: Question[] = [
  {
    ...
    description: 'Write a function to perform binary search on a sorted array. The function should return the index of the target element or -1 if not found.',
  ...
  },
  {
    ...
    description: 'Describe the purpose and usage of useState and useEffect hooks in React. Provide examples.',
    answerType: 'essay',
    departmentId: 'd1', // Engineering
    difficulty: 'intermediate',
    skillType: 'technical',
    ...
    marks: 10,
    rubric: 'Clear explanation (20pts), Examples (20pts), Best practices (10pts)',
    createdBy: 'admin',
  ...
  },
  {
    ...
    description: 'Design a URL shortening service like bit.ly. Include architecture, database schema, and API endpoints.',
    answerType: 'essay',
    departmentId: 'd1', // Engineering
    difficulty: 'advanced',
    skillType: 'technical',
    ...
    marks: 10,
    rubric: 'Architecture (30pts), Database design (25pts), API design (25pts), Scalability considerations (20pts)',
    createdBy: 'admin',
  ...
  },
  {
    ...
    description: 'Rate your ability to solve complex technical problems under pressure.',
  ...
  },
  {
    ...
    description: 'Upload a PDF containing your best 3 projects with descriptions.',
  ...
  },
  {
    ...
    description: 'Create a RESTful API for a basic todo application with endpoints for CRUD operations.',
    answerType: 'code_editor',
    departmentId: 'd1', // Engineering
    difficulty: 'advanced',
    skillType: 'technical',
    ...
    marks: 10,
    codeTemplate: '// Create your REST API endpoints here\n// Use Express.js or your preferred framework',
    createdBy: 'admin',
...
export default function QuestionsPage() {
  ...
  const [typeFilter, setTypeFilter] = useState<'all' | Question['answerType']>('all');
  const [difficultyFilter, setDifficultyFilter] = useState<'all' | Question['difficulty']>('all');
  const [skillFilter, setSkillFilter] = useState<'all' | Question['skillType']>('all');
  ...
  const filteredQuestions = questions.filter((q) => {
    ...
    const matchesSearch =
      ...
      (q.tags && Array.isArray(q.tags) && q.tags.some((tag) => tag.toLowerCase().includes(searchQuery.toLowerCase())));
        ...
        <div className="card p-4 bg-primary-50 border-primary-200">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button className="btn-outline text-sm text-danger-600">Delete Selected</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="card overflow-hidden">
          ...
          <table className="w-full min-w-full table-fixed">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="text-left text-xs font-medium text-gray-500 uppercase px-3 py-3 w-[35%]">Question</th>
                <th className="text-left text-xs font-medium text-gray-500 uppercase px-3 py-3 w-[12%]">Type</th>
                <th className="text-left text-xs font-medium text-gray-500 uppercase px-3 py-3 w-[10%]">Difficulty</th>
                <th className="text-left text-xs font-medium text-gray-500 uppercase px-3 py-3 w-[10%]">Skill</th>
                <th className="text-left text-xs font-medium text-gray-500 uppercase px-3 py-3 w-[8%]">Points</th>
                <th className="text-left text-xs font-medium text-gray-500 uppercase px-3 py-3 w-[10%]">Tags</th>
                <th className="text-right text-xs font-medium text-gray-500 uppercase px-3 py-3 w-[10%]">Actions</th>
              </tr>
            </thead>
                ...
                <tr key={question.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-3 py-4">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 truncate mt-1" title={question.description}>
                  ...
                  <td className="px-3 py-4">
                    <span className={`badge ${difficultyColors[question.difficulty]} capitalize text-xs whitespace-nowrap`}>
                      {question.difficulty}
                    </span>
                  </td>
                  <td className="px-3 py-4">
                    <span className={`badge ${skillTypeColors[question.skillType]} capitalize text-xs whitespace-nowrap`}>
                      {question.skillType}
                    </span>
                  </td>
                  <td className="px-3 py-4">
                  ...
                  <td className="px-3 py-4">
                    <div className="flex flex-wrap gap-1">
                      {question.tags.slice(0, 1).map((tag) => (
                        <span key={tag} className="badge badge-gray text-xs truncate max-w-[60px]" title={tag}>
                          {tag}
                        </span>
                      ))}
        ...
        <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button className="btn-outline px-3 py-1 text-sm" disabled>Previous</button>
            <button className="px-3 py-1 text-sm bg-primary-600 text-white rounded-lg">1</button>
            <button className="px-3 py-1 text-sm text-gray-600 hover:bg-gray-100 rounded-lg">2</button>
